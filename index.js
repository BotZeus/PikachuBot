...
async function starts() {
	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Ola@${num.split('@')[0]}\nBem vindo ao grupo aqui so fica quem transa *${mdata.subject}*`
				...
				teks = `Adeus @${num.split('@')[0]}vai pela sombra que bosta no sol seca👋`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
				wait: '⌛ Espere esta gerando, se nao der nao reclame dalem de fazer gratis⌛',
				...
				only: {
					...
					ownerB: '[❗] Este comando s� pode ser usado pelo proprietario do bot! ❌',
					...
					Badmin: '[❗] Este comando s� pode ser usado quando o bot se torna administrador! ❌',
                                        daftarB: `──「 Registre-se 」──\nOla mano !\nVoce nao esta registrado no banco de dados, \n\nComando : ${prefix}daftar nama|umur\nDigite : .daftar nama|lorde screamo ${prefix}`,
				}
			}
    			const apakah = ['Sim nao']
        		const bisakah = ['Sim, voce nao pode']
               ...
               break
				case 'demote':
                  ...
                  case 'promote':
					...
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
						teks = 'Removido com sucesso, Vaza daqui :\n'
						for (let _ of mentioned) {
							teks += `@${_.split('@')[0]}\n`
						}
						mentions(teks, mentioned, true)
						...
						mentions(`Pedido recebido, adicionar posi��o como administrador : @${mentioned[0].split('@')[0]}`, mentioned, true)
						client.groupMakeAdmin(from, mentioned)
					}
					break
				  case 'wa.me':
  ...
  client.updatePresence(from, Presence.composing) 
    ...
    client.sendMessage(from, options, text, { quoted: mek } )
			...
			case 'quotes':
              ...
              case 'owner':
                case 'creator':
                  client.sendMessage(from, {displayname: "Jekff", vcaurd: vcaurd}, MessageType.contact, { quoted: mek})
               client.sendMessage(from, 'Aqui est� o n�mero da minha dona mana, salva mana, salva depois',MessageType.text, { quoted: mek} )
                break
	case 'hidetag':
                client.updatePresence(from, Presence.composing) 
                if (!isUser) return reply(mess.only.daftarB)
		...
		case 'spamcall':
               client.updatePresence(from, Presence.composing)
				...
				case 'ytmp3':
					...
					teks = `❏ *Titulo* : ${anu.title}\n❏ *Tamanho* : ${anu.filesize}\n\nEspere Um Pouco Mano, Audio Esta Sendo Enviado Novamente...`
				...
				case 'ytmp4':
					...
					if (anu.error) return reply(anu.error)
					teks = `*❏ Titulo* : ${anu.title}\n\n*O VIDEO ESTA ENVIANDO, NAO ENVIE SPAM PAI*`
					thumb = await getBuffer(anu.thumb)
					client.sendMessage(from, thumb, image, {quoted: mek, caption: teks})
					buffer = await getBuffer(anu.result)
	...
	case 'kbbi':
          ...
          contextInfo: { mentionedJid: [from] }
           ...
           break
				...
				case 'add':
					...
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					if (args.length < 1) return reply('A que voc� deseja adicionar um genio??')
					if (args[0].startsWith('08')) return reply('Use o codigo do pais, mas')
					try {
				...
				case 'kick':
					...
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
						teks = 'Vaza daqui  Removi esse satanas com sucesso :\n'
						for (let _ of mentioned) {
							teks += `@${_.split('@')[0]}\n`
						}
						mentions(teks, mentioned, true)
						...
						mentions(`Pedido recebido, vaza daqui diabo  : @${mentioned[0].split('@')[0]}`, mentioned, true)
						client.groupRemove(from, mentioned)
				...
				case 'exe':
	              ...
	              if (!isOwner) return reply(mess.only.ownerB)
	               ...
	               exec(cmd, (err, stdout) => {
		           if(err) return client.sendMessage(from, "Comando Incorreto", text, { quoted: mek })
		           if (stdout) {
				...
				case 'ocr':
					...
					break
                     ...
                     const bug = body.slice(5)
                      if (pesan.length > 300) return client.sendMessage(from, 'Desculpe, o texto � muito longo, maximo de 300 textos', msgType.text, {quoted: mek})
                    ...
                    reply('O problema foi relatado ao propriet�rio do BOT, relatorio falso/main2 nao sera respondido.')
                    break
               case 'apakah':
               client.updatePresence(from, Presence.composing) 
	    ...
	    case 'kapankah':
			...
			case 'closegc':
					...
					var nomor = mek.participant
					const close = {
					text: `Grupo fechado pelo ademiro @${nomor.split("@s.whatsapp.net")[0]}\nagora *so os pika* das galaxias pode falar `,
					contextInfo: { mentionedJid: [nomor] }
					}
                ...
                case 'bukagc':
					...
					if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					open = {
					text: `Grupo aberto pelo administrador @${sender.split("@")[0]}\nagora *os membros comum* podem falar `,
					contextInfo: { mentionedJid: [sender] }
					}
					...
					if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
						...
						await ffmpeg(`./${media}`)
							...
							.on('error', function (err) {
								...
								reply(`❌ Falhou, no momento da conversao ${tipe} ke stiker`)
							})
							.on('end', function () {
								console.log('Finish')
				...
				case 'toimg':
                	...
                	client.updatePresence(from, Presence.composing) 
                        if (!isUser) return reply(mess.only.daftarB)
					if (!isQuotedVideo) return reply('❌ responda o video um ❌')
					reply(mess.wait)
					encmedia = JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo
					media = await client.downloadAndSaveMediaMessage(encmedia)
					ran = getRandom('.mp4')
               ...
               if (anu.error) return reply(anu.error)
                ...
                break
                   ...
                   break
					...
					if (args.length < 1) return reply('digite 1 para ativar Boas vindas neste Grupo')
					if (Number(args[0]) === 1) {
						if (isWelkom) return reply('o recurso est� ativo')
						welkom.push(from)
						fs.writeFileSync('./database/json/welkom.json', JSON.stringify(welkom))
						reply('❬ SUCESSO ❭ ative o recurso de boas-vindas neste grupo')
				...
				if (isUser) return reply(mess.only.daftarB)
					...
					client.sendMessage(from, 'Questao : *'+hobby+'*\n\nResponda : '+ by, text, { quoted: mek })
					break
                                case 'nsfwneko':
				    try{
						if (!isNsfw) return reply('❌ *NSFW ESTA DESLIGADO* ❌')
                                                if (!isUser) return reply(mess.only.daftarB)
				...
				case 'logowolf':
					...
					if (args.length < 1) return reply(`onde esta o texto? exemplo ${prefix}logowolf Nazwa|Canss`)
					reply(mess.wait)
            ...
            break
                                ...
                                case 'nsfwtrap':
                                        try{
                                                ...
                                                buffer = await getBuffer(res.result)
                                                client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Aqui esta a foto, mana...'})
                                        } catch (e) {
                                                console.log(`*Error* :`, color(e,'red'))
                                                reply('❌ *ERROR* ❌')
                                        }
			       ...
			       reply(anu.result.jadwalTV)
					break
                                ...
                                case 'coffee':
                                        ...
                                        buffer = await getBuffer(anu.result)
                                        client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Aqui esta a foto, fofo ...'})
                                        break
                                case 'lovepaper':
                                        var gh = body.slice(10)
                                        var teks1 = gh.split("|")[0];
                                        if (args.length < 1) return reply('onde esta o texto hum\nExemplo: ${prefix}lovepaper tiocafe')
                                        if (!isUser) return reply(mess.only.daftarB)
                                        reply(mess.wait)
                                ...
                                case 'mutgrass':
                                        var gh = body.slice(9)
                                        var teks1 = gh.split("|")[0];
                                        if (args.length < 1) return reply('onde esta o texto hum\nExemplo: ${prefix}mutgrass screamo')
                                        if (!isUser) return reply(mess.only.daftarB)
                                        reply(mess.wait)
                                ...
                                case 'randomkpop':
                                        ...
                                        buffer = await getBuffer(anu.result)
                                        client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Aqui kpop ok kpop...'})
                                        break
                                case 'fml2':
				...
				case 'simi':
					...
					//if (anu.error) return reply('Simi N�o sei mana')
					reply(anu)
					break
				case 'simih':
					if (!isGroup) return reply(mess.only.group)
					...
					if (Number(args[0]) === 1) {
						...
						reply('Ativar o modo simi com sucesso neste grupo ✔️')
					} else if (Number(args[0]) === 0) {
						samih.splice(from, 1)
						fs.writeFileSync('./src/simi.json', JSON.stringify(samih))
				...
				case 'clone':
					...
					break
                                ...
                                case 'pubglogo':
                                        ...
                                        var teks1 = gh.split("|")[0];
                                        var teks2 = gh.split("|")[1];
                                        if (args.length < 1) return reply('onde est� o texto um \ nExemplos: ${prefix}pubglogo Nazwa|Canss')
                                        if (!isUser) return reply(mess.only.daftarB)
                                        ...
                                        buffer = await getBuffer(anu.result)
                                        client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Aqui est� o logotipo, mana...'})
                                        break
                                case 'herrypotter':
                                case 'harrypotter':
                                        var gh = body.slice(12)
